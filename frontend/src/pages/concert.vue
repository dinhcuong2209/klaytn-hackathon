...
<script>
    import concerts from '@/api/concert';
    ...
    export default {
        ...
        methods: {
            ...
            async getData() {
                try {
                    const res = await concerts.getConcerts(this.filter);
                    ...
                    if (res && res.data && res.data.data && res.data.data.items) {
                        this.data = res.data.data.items.map((item) => ({
                            ...item,
                            startTime: dayjs(item.startTime).format('ddd DD MMM'),
                            dayOfWeek: dayjs(item.startTime).format('dddd'),
                            time: dayjs(item.startTime).format('h:mm A'),
                        }));
                    }
                } catch (error) {
